...
- **Settings backups preload via `--backup file.zip`** (or `BACKUP_ZIP=...`): the script uninstalls first (wiping the data container + cfprefsd cache), reinstalls, then copies the backup's `preferences.plist` to `<container>/Library/Preferences/<BUNDLE_ID>.plist` and `group.plist` to `…/group.com.christianselig.apollo.plist`. In the simulator both the app and app-group domains live in the app data container (the group suite falls back there without full app-group provisioning), matching the tweak's own `NSHomeDirectory()`-based restore path. The account's OAuth blobs and API keys load on launch, so the app comes up signed in — real, logged-in features can be exercised. **A backup contains live account credentials; keep test zips out of the repo** (`./.sim/` is gitignored).
