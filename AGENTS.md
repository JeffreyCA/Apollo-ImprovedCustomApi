...
- **Liquid Glass**: the checked-in `apollo-base.ipa` is itself already glass-bumped (its main binary reads `sdk 19.0` — check with `vtool -show-build`), so a plain `scripts/run-in-sim.sh` without `--glass` still yields a glass-linked app; `--no-glass` can't undo that, since the re-prep it triggers just re-extracts the same glass-bumped base. `--glass` (or `GLASS=1`) prepares the shell from a `patch.sh --liquid-glass` base — the canonical glass patcher, reused so it stays in sync: it bumps the main binary's linked SDK to iOS 26 (which flips `IsLiquidGlass()`, since `GetLinkedSDKVersion()` reads that field), drops the duplicate `@executable_path/Frameworks` LC_RPATH (iOS-26 dyld rejects it — otherwise the launch dies with an `SBMainWorkspace` denial), swaps in the prebuilt `Assets.car`, and writes the `CFBundleAlternateIcons` metadata that turns on the in-app icon picker. The glass base is cached at `./.sim/glass-base.ipa` (regenerated only when `apollo-base.ipa` changes); the shell's glass state is detected from the cached main binary's SDK (major >= 19). **For a true non-glass shell** (`IsLiquidGlass()` NO — UIKit's iOS-26 chrome and the tweak's LG hooks dormant), downgrade the prepped main binary in place: `vtool -set-build-version 7 15.0 16.0 -replace -output .sim/Payload/Apollo.app/Apollo .sim/Payload/Apollo.app/Apollo`, then `codesign -f -s -` the binary and then `.sim/Payload/Apollo.app`, then relaunch with `scripts/run-in-sim.sh --no-build` — the downgraded SDK reads as non-glass, matching the default `GLASS=0`, so later runs keep the shell.
